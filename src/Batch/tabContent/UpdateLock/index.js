...
import NoUpdateLockModel from '../../models/UpdateLock';
...
@model(NoUpdateLockModel)
@binding

export default class NoUpdateLock extends Component {
  componentDidMount() {
          ...
          <Col span={24} style={{ padding: 0 }}>
            {
              site() == 'intl' ? (<BatchMethod getData={this.model.getBatchMethodData}
                uploadDone={this.model.uploadDone} />) :
                (<BatchMethodCN getData={this.model.getBatchMethodData} uploadDone={this.model.uploadDone} />)
            }
          </Col>
        </Row>
        <Row className='bulk-op-item'>
          ...
          <Col md={18} className="changeWidthStyle">
            <RadioGroup
          ...
          <Col span={24} style={{ padding: 0 }}>
            {
              site() == 'intl' ? (<IdentifyEmail onChange={this.model.vCodeChange}
                url='/updateLockApi/SendAuthCode.json' />) :
